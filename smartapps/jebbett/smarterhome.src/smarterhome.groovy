 ...
 *	others their code has been an insiration or I have used snippets of their code to support this project where the licence has supported.
 *
 ...
 * VERSION CONTROL
 *
 *	V0.0.1	03/10/16	PRE-RELEASE (NOT FOR DISTRIBUTION)
 *
 *	
 */

def version() {	return "v0.0.1 - DO NOT DISTRIBUTE" }
...
private pageMainControl() {
	dynamicPage(name: "pageMainControl", title: "", install: remoteDefault, uninstall: false) {
        ...
        section(){
            ...
            paragraph("Add Scenes and Notifications using the option under Triggers / Restrictions")
        }
	}
}
...
private pageModules(params) {
	dynamicPage(name: "pageModules", title: "Modules", install: false) {
        ...
        section("About Modules") {paragraph("Modules do the automatic things to make your life easier")}
        ...
        switch (settings.modType) {
			...
			case "Notification":
        		section("External Triggers") {
                    ...
                    input "remoteEnabled", "bool", title: "Add to Remote Control", defaultValue: false, required: false
    			}    
        	break
		}
...
def pageTimeZones(params) {
	dynamicPage(name: "pageTimeZones", title: "Time Modes", install: false, uninstall: false) {
		section(){
            ...
            input "tSetSTMode", "bool", title: "SmarterHome Sets Mode In ST Every 15 Minutes", defaultValue: false, submitOnChange: true, required: false
        }
        ...
        section() {
			paragraph("Here you can specify a Mode to be returned at different times to allow different things to happen around your house.\n\nSelectable modes are taken from the SmartThings list of Modes\n\nThis will be checked from the top of the list to the bottom and stop at the first match so days must not overlap.", title: "About TimeZones", required: true, state: "complete")
		}
	}
}
...
def getTimeMode() {
	...
	def d = 1
	while (true) {
    	if(settings."dayOfWeek$d" != null) {d++}
        else {d=d-1; break}
    }
    ...
    for (int i = 1; i <= d; i++) {
		if(settings."dayOfWeek$i" in okDays ){tz=i; break}
	}

    //Find how many time bands there are (z) for day band
	def z = 1
	while (true) {
    	if(settings."timeSType$tz$z" != null) {z++}
        else {z=z-1; break}
    }
    //If no timezones found exit and return "None"
    if(z==0){return "None"}
...
// Play, Pause or Stop
	switch (mStatus) {
        ...
        break;    
        case "paused":
           	if(mPauseDelay == "0"){mPauseCommand()}else{
        		runIn(mPauseDelay, mPauseCommand)
        	}
        break;
        case "stopped":
          	if(mStopDelay == "0"){mStopCommand()}else{
        		runIn(mStopDelay, mStopCommand)
        	}
        break;
	}
}
...
// PLAY PAUSE STOP
def mPlayCommand(){
	if (settings?.mPlayScene){
    	mPlayScene?.each { activity -> parent.triggerChild(activity,null) }
    }	
}
def mPauseCommand(){
	if (settings?.mPauseScene){
    	mPauseScene?.each { activity -> parent.triggerChild(activity,null) }
    }	
}
def mStopCommand(){
	if (settings?.mStopScene){
    	mStopScene?.each { activity -> parent.triggerChild(activity,null) }
    }	
}
	...
	dynamicPage(name: "pageNDev", install: false, uninstall: false) {
        ...
        section("Mobile SMS"){
            ...
            if(nSMSMsg){
            	...
            	//input "nRecipients", "contact", title: "Send notifications to" // when contact book enabled then this will allow combination of SMS and app notifications
        	}
       }
	...
	dynamicPage(name: "pageNLights", install: false, uninstall: false) {
	    section() {
            ...
            if(nLightType == "Flash Colour"){
                ...
                input "nRGBColor", "enum", title: "And This Colour", required: false, multiple: false, submitOnChange: true,
            ...
            if (nLightType){
    	        ...
    	        input "nGap", "number", range: "0..${10000/nFlash}", title: "This Frequently (ms) MAX:${10000/nFlash}", defaultValue: "600"
			}
    	}
	}
}
...
def executeNotification(value){
    ...
    if(settings?.nCancelSecs){
    	runIn(nCancelSecs,nExpiredNotification)
    }
    
	// Scedule notification if delay set.
    if(settings?.nDelaySecs){
    	runIn(nDelaySecs,nNotificationEvent)
        return
    }
    
    // If no delays or secondary triggers set then notify
    if(!nDelaySecs && !nMotion && !nContact){
    	nNotificationEvent()
    }
}
...
def nNotificationEvent(){
    ...
    if(parent.notificationInProgress("get")=="yes"){
        ...
        else if(nQueueType == "Drop"){
        	logWriter("Notification DROPPED as another notification is in progress.")
            return
        }
        else if(nQueueType == "Force" || nQueueType == "Bypass"){
        	logWriter("Notification FORCED while another notification is in progress.")
        }
    }
...
//// GET DYNAMIC MESSAGE IF SET
    ...
    try {
    	if(nDyn1){
        	dynamicMsg = generateMsgText(state.storedValue)
    	}
	}
	catch (Throwable t) {
		log.error t
...
//// APP NOTIFICATION
	if(nAppMsg){
    	if(nAppType == "Custom Message"){
    		sendPush(nMessageText)
        }else{
        	sendPush("$dynamicMsg")
        }
    }
...
//// SMS NOTIFICATION
	if(nSMSMsg){
    	if(nSMSType == "Custom Message"){
    		sendSms(nPhone, nMessageText)
        }else{
        	sendSms(nPhone, dynamicMsg)
        }
    }
    
//// ALARM NOTIFICATION    
    
	// Trigger alarm for x seconds      
	if(settings?.nAlarms){
		nAlarms?."${nAlarmType}()"
		runIn(nAlarmSeconds,silenceAlarm)
	}    
...
//// SET AS NOT RUNNING AFTER 20 SECONDS - unless audio longer
    ...
    if(nAudio){
    	if(soundToPlay.duration != null){
        	...
        	if(soundDur >= 21){
    			pleaseWait = "${soundToPlay.duration}" as int
        	}
    	}
    }
...
//// LIGHTS
    if(nLightType){
    	...
    	switch (settings.nLightType) {
            ...
            // Get current value
            nSwitches?.each { item ->
                saveVals << [deviceID:item.id, switch: item.switchState.value]
            }
            // Now flash 
            def d = 0
            ...
            case "Flash Colour":
            	// Get current value
                nRGBBulbs?.each { bulb ->
                    saveVals << [deviceID:bulb.id, hue: bulb.hueState?.value, saturation: bulb.saturationState?.value, level: bulb.levelState?.value, switch: bulb.switchState?.value]
                }
                ...
                def d = 0
   				for (int a = 1; a <= flashes; a++) {
               		...
               		pause(delay)
                    nRGBBulbs?.setColor([level: 5])
                    pause(delay)
                }    
                
                //return to previous value
                nRGBBulbs?.each { bulb -> 
	            	bulb.setColor(returnHueSettings(bulb.id, saveVals))
            	}
    		break
		}
    }
    ...
    for (int i = 1; i <= 10; i++) {
        ...
        if(settings."nDyn$i" != null){
            ...
            case "The weather":
                ...
                break;
            case "CUSTOM MESSAGE":
            	msgAdd = settings.nMessageText
                break;
            case "SYSTEM MESSAGE":
            	someWords = "$notificationVal"
                if("$notificationVal" == "null") {msgAdd = ""} else {msgAdd = "${someWords} "}
                break;
...
def pPresenceEvent(evt){
	...
	if(evt.value == "present"||evt.value == "on") {
        ...
        // Set notification to trigger
        if (pNotifyArrive){
    		pNotifyArrive?.each { activity -> 
       			parent.triggerChild(activity,settings.appName)
    		}
    	}  
	}
    	...
    	section("Lights") {
        	...
        	if(settings."sRgbBulbs$i"||settings."sDimmers$i") {
        		...
        		if(settings."sRgbBulbs$i") {
        			input "sBColor$i", "enum", title: "And This Colour", required: false, multiple: false, submitOnChange: true,
                    options: colorOptions()
            	}
...
def executeScene(evt){
    ...
    if("off" == evt.value){
    	if (sTriggerSceneOff){
    		sTriggerSceneOff?.each { activity -> 
       			parent.triggerChild(activity,null)
    		}
    	}
        return
	}
    ...
    if(sLightMeter){
    	...
    	if(curLight >= reqLight) {
        	logWriter("Exited as too light")
            return
    	}
    }
	...
	while (settings."sSceneMode$d" != null && d !=0) {	
    	settings."sSceneMode$d".each { scene -> 
        	if(scene == tm || scene == "Any"){
          		i = d
        	}
    	}
        if(i>0){d = 0}else{d++}
	...
	if ("sBLevel$i" != null) {
        ...
        if (sDimOnlyIfOn$i){
            ...
            settings."sRgbBulbs$i"?.each { hue -> if ("on" == hue.currentSwitch) {hue.setColor(hueSetVals)}}
        }else{
    ...
    if (settings?.sTriggerSceneCancel){
    	sTriggerSceneCancel?.each { activity -> 
    		parent.triggerChild(activity,"cancel")
    	}
    }
}
...
def showSecZone(params){
    ...
    section("Vacation Mode") {
    	href(name: "pageSecVacation", title:"", description: "Vacation Mode", page: "pageSecVacation", state: (secVacationEnabled? "complete" : null), image: getIcon("holiday"))
    }
}
...
def pageSecVacation(){
	dynamicPage(name: "pageSecVacation", install: false, uninstall: false) {
	    section("Vacation Mode") {
            ...
            input "secSceneVacation", "enum", title: "Using These Scenes / Notifications Ramdomly", required: false, multiple: true, description: "", options: parent.moduleList(app.id, 'Scene, Notification')
            input "secVacationModes", "enum", title: "Only In These Modes", required: true, multiple: true, defaultValue: "Any", options: getModeList()
            input "secSceneVacationOff", "enum", title: "Otherwise Set These Off Scenes", required: false, multiple: true, description: "", options: parent.moduleList(app.id, 'Scene, Notification')
    	}
	}
}
...
def executeSecurityMode(value){
	...
	if(value == "on"){
    	parent.securityState(settings.appName)
    }
    else if(value == "refresh"){
    	if(settings?.secVacationEnabled){executeVacation()}
    }
}

def executeVacation(){

    // TODO: If in secVacationModes then randomly set one of these secSceneVacation
    // else set secSceneVacationOff
}
...
def pageSecurity(){
	dynamicPage(name: "pageSecurity", install: false, uninstall: false) {
        ...
        section("Security & Safety") {
            ...
            if(inDev) href(name: "pageSecFireFlood", title:"", description: "Fire & Flood", page: "pageSecFireFlood", state: null, image: getIcon("fireflood"))
        }
...
def pageSecFireFlood(params){	
    dynamicPage(name: "pageSecFireFlood", install: false, uninstall: false){ 
        section("*** THIS IS NOT YET CODED ***")
        section("Detect Fire & CO2"){
        	input "secSmoke", "capability.smokeDetector",title: "Smoke Detectors", multiple: true, required: false, submitOnChange: true
            input "secCO2", "capability.carbonMonoxideDetector",title: "Carbon Monoxide Detectors", multiple: true, required: false, submitOnChange: true
        	input "secTemp", "capability.temperatureMeasurement",title: "Temperature Sensors", multiple: true, required: false, submitOnChange: true
            if(secSmoke||secCO2||secTemp){
            	input "secTempValue", "number", title: "For this many seconds", required: true, defaultValue: "120"
            	paragraph ("Then Do This")
            	input "secFireAlarm", "bool", title: "Trigger Alarm", defaultValue: "false", required: "false"
            	input "secFireScene", "enum", title: "Trigger This Scene", required: false, description: "", multiple: true, options: moduleList(app.id, 'Scene')
            	input "secFireNotification", "enum", title: "Trigger This Notification", required: false, description: "", multiple: true, options: moduleList(app.id, 'Notification')
        	}
        }
        
        
        section("Detect Flooding") {
			input "secWater", "capability.waterSensor",title: "Water Sensors", multiple: true, required: false, submitOnChange: true
            if(secWater){
            	paragraph ("Then Do This")
	            input "secValve", "capability.valve",title: "Turn Off These Valves", multiple: true, required: false
    	        input "secFireAlarm", "bool", title: "Trigger Alarm", defaultValue: "false", required: "false"
        	    input "secWaaterScene", "enum", title: "Trigger This Scene", required: false, description: "", multiple: true, options: moduleList(app.id, 'Scene')
            	input "secWaterNotification", "enum", title: "Trigger This Notification", required: false, description: "", multiple: true, options: moduleList(app.id, 'Notification')
        	}
    	}        
	}
}
...
def setSecurityState(data) {
    ...
    childApps.each { child ->
		if(child.modType == "Security" && child.appName == data.status) {
        	location.helloHome?.execute(child.routineArmed)
        }
	}
}
...
def securityPresence(name,event){
	...
	if(event == "present" || event == "on"){
    	...
    	if(settings?.clearLastPersonDelay != "null" && settings?.clearLastPersonDelay >= "1"){
        	runIn(1200,clearLastPerson)
   		}
    }
    ...
    if(peopleHome){
    	...
    	// Set alarm, if auto arming is turned on.
        if(secAutoArm){
    		triggerChild(secAutoArmMode,"on")
        }
    }
}
...
// return last person
def secLastPerson(){
    def lp = state.lastPerson
    //clear last person after 30 seconds
...
def secDoorHandler(evt){
	def doorsOpen = []
    if(!state.openDoors) state.openDoors = "false"
    
    secDoorContact.each { contact ->
    	if(contact.contactState == "open"){
        	doorsOpen << contact.label
        }
    }
    
    if(doorsOpen == null){
    	unschedule(secDoorAlert)
        state.openDoors = "false"
    }else if(state.openDoors == "false"){
    	state.openDoors = doorsOpen
    	runIn(secDoorDelay,secDoorAlert)
    }
}

def secDoorAlert(params){
	if (secDoorNotification){
    	secDoorNotification?.each { activity -> 
    		triggerChild(activity,secDoorNotification)
    	}
    }
}
...
def zMotionDetected(evt) {
    ...
    if(zOffDelay){
    	//if(evt.value == "inactive" || evt.value == "closed"){
	    if(evt.value in ["inactive", "closed"]){
        	runIn(zOffDelay,zMotionExpired)
        }
    }
...
def zMotionExpired(evt) {
    ...
    if(zDisableSwitch){
    	if(zDisableSwitch.switchState.value == "on"){
    ...
    state.zMotionAlive = false
    logWriter("No montion detected for defined time")
    if (settings?.zSceneOff){
    	zSceneOff?.each { activity -> 
        	parent.triggerChild(activity,settings.appName)
    	}
	}
}
...
private def logWriter(value) {
	if(parent){
    	if(parent.debugLogging) {log.warn "${app.label} >> ${value}"}
    }else{
		if(debugLogging) {log.warn "${app.label} >> ${value}"}
    }
}
...
def getRoutines() {
    ...
    location.helloHome?.getPhrases().each { routine ->
        result << ["${routine.id as String}" : "${routine.label as String}"]        
	}
	return result.sort { a, b -> a.value.toLowerCase() <=> b.value.toLowerCase() }
}
...
def remoteList() {
	def result = [:]
	childApps.each { child ->
		if(child.remoteEnabled){result << ["${child.id as String}" : "${child.label as String}"]}
	}
	return result.sort { a, b -> a.value.toLowerCase() <=> b.value.toLowerCase() }
}
...
def getIcon(value){
	switch (value) {
		case "home":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__home.png" ;break
        case "remo":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__remote.png" ;break
		case "cp":			return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__Control-Panel.png" ;break
		case "apps":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__Apps.png" ;break
		case "sec":			return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__LOCK.png" ;break
		case "info":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__info.png" ;break
        case "plus":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__plus.png" ;break
        case "play":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__WMP.png" ;break
        case "calendar":	return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__calendar.png" ;break
        case "rules":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__tasks.png" ;break
        case "message":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__mail.png" ;break
        case "Noti":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__msg.png" ;break
        case "Scen":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__light.png" ;break
        case "fireflood":	return "https://dl.dropboxusercontent.com/u/27382557/icons/MB_Fire.png" ;break
        case "holiday":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__world.png" ;break
        case "eye":			return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__viewer.png" ;break
        case "temp":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB_Temperature.png" ;break
        case "time":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__clock.png" ;break
        case "weather":		return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__weather.png" ;break
        case null:			return "https://dl.dropboxusercontent.com/u/27382557/icons/MB__Control-Panel.png" ;break
	}
}
...
def manualTrigger(){
	// Trigger scene
    executeChild()
    //return to page you were on
    pageAdvanced()
}

// EXIT if disabled
def disabledCheck(){
	if(settings?.disabled != null && disabled.currentSwitch == "on") {
    	logWriter("Disabled by switch")
        return true
    }
    def timeMode = parent.getTimeMode()
    if (settings?.activeMode != null && !activeMode.contains("Any") && !activeMode.contains(timeMode)) {
    	logWriter("Disabled by invalid mode")
        return true
    }
    return false
}
...
def recurringChecks(){
    ...
    // Update ST Mode
    setSTMode()
    // Refresh security children - For holiday mode.
    moduleList(app.id, 'Security').each { activity ->
        triggerChild(activity.key,"refresh")
    }
    return
}
...
def updateLog(command, name, length, event){
    ...
    if(length == null || length == 0){state.remove(logName); return "State Cleared"}		// If length set to 0, delete state
...
def sendChildAppLink(){
	// Get child app details, but providing the format is the same, then this could be any unique ID and descriptive name.
    // You could add a flag in the child to state if you want to publish or not to avoid huge lists.
    // child.id is preferable as is always unique and does not change if label or name changes.
	def result = [:]
	childApps.each { child -> if(child.remoteEnabled) result << ["$child.id" : "$child.appName"]}
